...
OSStatus AudioInputProc( void* inRefCon, AudioUnitRenderActionFlags* ioActionFlags, const AudioTimeStamp* inTimeStamp, UInt32 inBusNumber, UInt32 inNumberFrames, AudioBufferList* ioData)
{
    ...
    //  NSLog(@"in AudioInputProc %u frames", inNumberFrames);
    ...
    int nsamples = 0;
    float avg = 0.0;
    if (afr->audioBuffer != NULL) {
        for(int i=0; i<afr->audioBuffer->mNumberBuffers; i++) {
            float *t = (float *)afr->audioBuffer->mBuffers[i].mData;
            // int ns = (float *)afr->audioBuffer->mBuffers[i].;
        //    NSLog(@"in AudioInputProc buffer #%d, %f %f %f %f %f %f", i, t[0], t[1], t[2], t[3], t[4], t[5]);
    ...
    // NSLog(@"avg = %f, samples = %d", avg, nsamples);
    
    if (nsamples > 0) {
        avg /= (nsamples / afr->audioBuffer->mNumberBuffers);
    }

    afr->m_lastamp = (afr->m_lastamp * 0.1) + (avg * 0.9);
    // afr->m_lastamp = avg;

    
/*
    NSLog(@"buf %@", afr->audioBuffer);
    */
    /*
